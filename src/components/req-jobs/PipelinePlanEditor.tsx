...
export function PipelinePlanEditor({ jobId }: { jobId: string }) {
  ...
  const applyTemplate = async (t: PlanTemplate) => {
    if (!confirm(`Apply “${t.name}”? This replaces this job's Active & Offer stages and the rules on the stages this template defines; rules on other stages are kept. Any candidates in a replaced stage move back to “Applied”.`)) return
    setTplBusy(true)
    try {
