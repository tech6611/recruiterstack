...
import { Plus, Trash2, ChevronUp, ChevronDown, Loader2, CalendarClock, Save } from 'lucide-react'
...
const blankRound = (): Round => ({
  name: '', interview_type: 'video', duration_minutes: 45, interviewer_role: '', stage_id: null,
})

// A sensible 3-round starting point for an empty plan.
const STARTER: Round[] = [
  { name: 'Recruiter Screen', interview_type: 'phone',     duration_minutes: 30, interviewer_role: 'Recruiter',      stage_id: null },
  { name: 'Technical Round',  interview_type: 'technical',  duration_minutes: 60, interviewer_role: 'Interviewer',    stage_id: null },
  { name: 'Hiring Manager',   interview_type: 'video',      duration_minutes: 45, interviewer_role: 'Hiring Manager', stage_id: null },
]
...
export function InterviewPlanTab({ jobId }: { jobId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    const fetched: InterviewPlanRound[] = planRes?.data?.rounds ?? []
    setRounds(fetched.map(r => ({
      name: r.name,
      interview_type: r.interview_type,
      duration_minutes: r.duration_minutes,
      interviewer_role: r.interviewer_role ?? r.interviewer_name ?? '',
      stage_id: r.stage_id,
    })))
      ...
      const res = await fetch(`/api/jobs/${jobId}/interview-plan`, {
        ...
        body: JSON.stringify({
          rounds: rounds.map(r => ({
            ...
            interview_type: r.interview_type,
            duration_minutes: r.duration_minutes,
            interviewer_role: r.interviewer_role.trim() || null,
    ...
    <Card>
      <CardHeader className="flex flex-row items-start justify-between gap-4">
        <div>
          ...
          <CardDescription>
            Define the rounds for this job — how many, of what kind, and who runs each. Scheduling will follow this plan.
          </CardDescription>
        </div>
      ...
      <CardContent className="space-y-3">
        {rounds.length === 0 ? (
          ...
          <>
              ...
              <div key={i} className="rounded-xl border border-slate-200 bg-white p-3">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex flex-col">
                    <button aria-label="Move up" onClick={() => move(i, -1)} disabled={i === 0}
                      className="text-slate-300 hover:text-slate-600 disabled:opacity-30"><ChevronUp className="h-4 w-4" /></button>
                    <button aria-label="Move down" onClick={() => move(i, 1)} disabled={i === rounds.length - 1}
                      className="text-slate-300 hover:text-slate-600 disabled:opacity-30"><ChevronDown className="h-4 w-4" /></button>
                  </div>
                  ...
                  <Input
                    ...
                    placeholder="Round name (e.g. Hiring Manager)"
                    className="flex-1"
                  />
                ...
                <div className="mt-3 grid grid-cols-1 gap-3 pl-10 sm:grid-cols-4">
                  ...
                  <div>
                    ...
                    <Input value={r.interviewer_role} onChange={e => update(i, { interviewer_role: e.target.value })} placeholder="Role or name" />
                  </div>
                  <div>
                    <Label className="mb-1 block text-[11px] uppercase tracking-wide text-slate-400">Duration (min)</Label>
                    <Input type="number" min={5} max={600} value={r.duration_minutes}
                      onChange={e => update(i, { duration_minutes: Number(e.target.value) || 0 })} />
                  </div>
                  <div>
            ...
            <Button size="sm" variant="outline" onClick={() => setRounds(rs => [...rs, blankRound()])}>
