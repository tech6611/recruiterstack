...
import { ChevronRight, ShieldAlert, MapPin, Building2, ThumbsUp, ThumbsDown, FileQuestion } from 'lucide-react'
        ...
        <thead>
          <tr className="bg-slate-50 text-slate-500">
            ...
            {icp.must_haves.map((m, i) => (
              <th key={m.id} className={`min-w-[72px] px-2 py-2 align-bottom text-[10px] font-medium leading-tight ${i === 0 ? 'border-l-2 border-slate-200' : ''}`}>
                {i === 0 && <span className="block text-[8px] font-bold uppercase tracking-wide text-slate-400">Must-have</span>}
                {m.label}
              </th>
            ))}
            {icp.competencies.map((c, i) => (
              <th key={c.id} className={`min-w-[80px] px-2 py-2 align-bottom text-[10px] font-medium leading-tight ${i === 0 ? 'border-l-2 border-slate-200' : ''}`}>
                <span className="block text-[8px] font-bold uppercase tracking-wide text-slate-400">{i === 0 ? `Competency · ${c.weight}%` : `${c.weight}%`}</span>
                {c.name}
              </th>
            ))}
                    ...
                    <div className="flex items-start gap-2.5">
                      ...
                      <button type="button" onClick={() => toggleOpen(m.candidate_id)} className="min-w-0 text-left">
                        <div className="flex items-center gap-1.5">
                          ...
                          {!passedGates && (
                            <span className="inline-flex shrink-0 items-center gap-0.5 rounded-full bg-red-100 px-1.5 py-0.5 text-[9px] font-semibold text-red-700">
                              <ShieldAlert className="h-2.5 w-2.5" />Missing must-have
                            </span>
                          )}
                          {m.data_incomplete && (
