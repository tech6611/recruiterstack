...
0) recruiter_brief — BEFORE anything else, decide WHICH specialist recruiter you are for this search, given the role, the level, the hiring company and its industry/stage, and the market. Name the niche precisely (e.g. "Strategy & Operations / BizOps recruiter for scaling SaaS, Bengaluru", not "business recruiter") and write the brief you would hand a junior on your desk:
   ...
   - adjacent_titles: the logical career-progression titles, IN ORDER (closest feeder first), you would step to once the exact title families run dry — the roles that genuinely FEED this one (a Tech Lead Manager search widens to a Staff Engineer who leads, then a Senior Staff Engineer; a Chief of Staff search widens to Strategy Manager, then Senior Consultant). Never generic level words alone ("Senior", "Manager").
   - target: the per-role SOURCING GOAL — how many qualified leads a healthy search for THIS seat should surface before you would stop widening, given how deep or thin this market is and how rare the profile is. A senior niche seat in a thin market might target ~20–40; a broad role in a deep market 100+. Give qualified_leads (an integer) and a one-line rationale. This is the yardstick for when a search level is "too thin" and must widen.
