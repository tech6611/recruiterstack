...
// For an application's canonical job, resolve the hiring manager to book against
// ONLY from the job itself (jobs.custom_fields.hiring_manager_email/name). The HM
// flows down from the approved requisition onto the job at creation time and is
// editable on the job, so the job is the single source of truth — this matches
// the Django prod sender, keeping both in sync. Returns the HM's { email, name },
// or null when the job has no HM set (the caller then uses the token's
