...
/** POST — add sourced candidates to this job's pipeline as applications
 *  (source: 'sourced'), into the first stage. Skips any already in the pipeline. */
  ...
  try {
    ...
    for (const candidateId of toAdd) {
      ...
      await supabase.from('application_events').insert({
        ...
        event_type: 'applied',
        note: 'Added to pipeline from Sourcing',
        created_by: 'Sourcing',
        org_id: orgId,
      } as never)
      added++
