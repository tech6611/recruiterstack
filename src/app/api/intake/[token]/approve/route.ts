...
import { approveCanonicalIntakeJob } from '@/modules/ats/domain/job-pipelines'

// GET /api/intake/:token/approve — one-click approval from email link
export async function GET(_req: NextRequest, { params }: { params: { token: string } }) {
  const supabase = createAdminClient()

  const result = await approveCanonicalIntakeJob(supabase, params.token).catch(() => null)
  ...
  return new NextResponse(
    ...
    <body style="font-family:system-ui,sans-serif;text-align:center;padding:80px 20px;background:#f0fdf4;">
      <div style="max-width:440px;margin:0 auto;">
        ...
        <p style="color:#374151;font-size:16px;">The Job Description for <strong>${result.position_title}</strong> has been approved and is ready to post.</p>
