...
### Changed
- **ICP generation deepened for the reasoned ladder (#2 prompt).** The recruiter-brief pass now asks for more, reasoned **feeder-pool tiers** (3–5, each with a `relationship`, ordered by priority — the companies a recruiter widens to at the same title), an **ordered career-progression** `adjacent_titles` (closest feeder first), and a new **per-role `target`** (a qualified-lead goal + rationale). This is the raw material the reasoned ladder consumes, and the "too thin?" yardstick for the coming adaptive planner (#3). Brief type + generation schema + prompt updated; tests added.
...
### Added
- **Ideal profile is now an editable tile grid** on the Scoring tab (replaces the read-only row list): each dimension is a card with a per-kind icon, a relax tag, and a pencil to edit it inline (city + radius for location, min/max for the years band, chip lists for titles/companies/etc.); employers show fetched company logos. Styled in the platform's pine/sand/gold palette. New `IdealProfileTiles.tsx` + shared `CompanyLogo.tsx`; edits write back through `mustHaveFromCriterion` so the ICP's must-haves stay the single source of truth.
- **Foldable "Review N profiles to calibrate" pop-up** (Juicebox-style) on the Source tab: a floating, collapsible card showing a diverse trio (via `pickCalibrationSet`) with 👍/👎 that flow through the existing `setSourcingDecision` engine; "Review all" opens the full diverse set in the matrix. New `CalibrationPopup.tsx`, mounted in `SourcingTab`; reuses the calibration engine (no new backend). Foldable via a chevron or the bottom grip; dismissible.
